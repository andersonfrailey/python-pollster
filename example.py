...
print('  Found %d charts; our results page has %d charts' % (charts.count, len(charts.items)))
...
print('  Found %d polls; our results page has %d polls and begins with %s' % (polls.count, len(polls.items), polls.items[0].slug))
print('  next_cursor is %s; using it in another GET request...' % polls.next_cursor)
...
print('  Got another %d polls, such as %s' % (len(polls2.items), polls2.items[0].slug))
...
print('  Poll conducted from: %s to: %s, entered into Pollster: %s' % tuple(t.isoformat() for t in [ poll.start_date, poll.end_date, poll.created_at ]))
print('  Questions: %d' % len(poll.poll_questions))
print('  First question name: %s' % poll.poll_questions[0].question.name)
print('  First question options: %s' % ', '.join(r.label for r in poll.poll_questions[0].question.responses))
print('  First question first sample subpopulation: %s' % poll.poll_questions[0].sample_subpopulations[0].name)
def simple_format_response(r):
    return '%s (%s): %0.1f' % (r.text, r.pollster_label, r.value)
print('  First question responses: %s' % '; '.join(simple_format_response(r) for r in poll.poll_questions[0].sample_subpopulations[0].responses))
...
print('  Found %d poll questions plotted on chart:' % len(chart_poll_questions))
...
print('  Found %d questions; our results page has %d questions' % (questions.count, len(questions.items)))
print('  First Question: %s (%s)' % (questions.items[0].slug, questions.items[0].name))
print('  Responses Pollster tracks: %s' % ', '.join(map(lambda r: r.label, questions.items[0].responses)))
...
print()
print('GET /question/:slug/poll_responses_raw.tsv... (again, for %s)' % question_slug)

responses_raw = api.questions_slug_poll_responses_raw_tsv_get(question_slug)

print('  Found %d response data points to Question %s' % (len(responses_raw), question_slug))
